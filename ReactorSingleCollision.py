...
class PWR:
    ...
    def __init__(self, n, dim, n_neutrons, speed, absorb_rate=0.022, seed=None, n_blocks=10 , plot_data=True):
        ...
        self.atoms = np.array([dim/2,dim/2]).reshape(-1,2)
        self.collided  = np.zeros((self.n))
        self.neutrons = np.array([dim/4,dim/4]).reshape(-1,2)
        self.directions = np.array([np.pi/4])
    ...
    def plot(self, i):
        ...
        self.plotter.scatter(self.atoms[:,0], self.atoms[:,1], lim, lim,s=5000, c=0)
